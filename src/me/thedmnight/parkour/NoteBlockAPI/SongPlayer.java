...
public abstract class SongPlayer {
	...
	public void addPlayer(Player p) {
		synchronized (this) {
			if (!playerList.contains(p.getName())) {
				...
				ArrayList<SongPlayer> songs = parkourMain.playingSongs.get(p.getName());
				if (songs == null) {
					songs = new ArrayList<SongPlayer>();
				}
				songs.add(this);
				parkourMain.playingSongs.put(p.getName(), songs);
			}
		}
	}
	...
	public void removePlayer(Player p) {
		synchronized (this) {
			...
			ArrayList<SongPlayer> songs = new ArrayList<SongPlayer>(parkourMain.playingSongs.get(p.getName()));
			songs.remove(this);
			parkourMain.playingSongs.put(p.getName(), songs);
